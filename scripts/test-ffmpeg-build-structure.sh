...
for requirement in 'vswhere.exe' 'VsDevCmd.bat' 'cl.exe' 'link.exe' 'lib.exe' 'cmake.exe' 'ninja.exe' 'MSYS2_LOCATION' 'OVAYRA_MSVC_BIN' 'OVAYRA_MSYS_BIN' 'OVAYRA_NATIVE_CMAKE' 'OVAYRA_NATIVE_NINJA' 'CC = '\''cl'\''' 'CXX = '\''cl'\''' 'AR = '\''lib'\''' 'LD = '\''link'\'''; do rg -F -- "$requirement" "$windows_ps1" >/dev/null; done
for requirement in 'PATH="/usr/bin:/ucrt64/bin:$PATH"' 'OVAYRA_MSYS_BIN' 'OVAYRA_NATIVE_CMAKE' 'OVAYRA_NATIVE_NINJA' 'make_cmd="$msys_bin/make.exe"' '"$make_cmd" -j' 'MSYS GNU make' 'OVAYRA_MSVC_BIN' 'PATH="$msvc_bin:$PATH"' 'env -u CC -u CXX -u AR -u LD ./configure' '--target=x86_64-win64-vs17' '"$cmake_cmd" -S' 'CMAKE_MAKE_PROGRAM="$ninja_win"' 'CMAKE_C_COMPILER=cl' 'CMAKE_CXX_COMPILER=cl' '--toolchain=msvc' '--enable-dxva2' 'changes.diff' '.ovayra-target'; do

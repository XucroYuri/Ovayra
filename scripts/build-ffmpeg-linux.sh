...
cmake_cmd=$(command -v cmake || true); [[ -x "$cmake_cmd" ]] || { echo 'CMake is required for the pinned Opus Git source' >&2; exit 65; }
...
cd "$source_root/ffmpeg"; configure=(--prefix="$stage_root" --pkg-config-flags=--static --disable-autodetect --disable-debug --disable-doc --disable-ffplay --disable-network --enable-ffmpeg --enable-ffprobe --enable-libopus --enable-libvpx --enable-version3 --disable-gpl --disable-nonfree --enable-vaapi --enable-libdrm --enable-ffnvcodec --enable-nvenc --enable-nvdec --extra-cflags="-I$dependency_prefix/include" --extra-ldflags="-L$dependency_prefix/lib"); pkg_config_dir="$dependency_prefix/lib/pkgconfig"
system_pkg_config_dirs=/usr/lib/x86_64-linux-gnu/pkgconfig:/usr/lib/pkgconfig:/usr/share/pkgconfig
if ! PKG_CONFIG_PATH="$pkg_config_dir" PKG_CONFIG_LIBDIR="$pkg_config_dir:$system_pkg_config_dirs" ./configure "${configure[@]}"; then
...
cp "$source_root"/{ffmpeg-8.1.2.tar.xz,ffmpeg-8.1.2.tar.xz.asc,libvpx-source.tar.zst,opus-source.tar.zst,nv-codec-headers-source.tar.zst,ffmpeg-signature-attestation.json} "$stage_root/provenance/"; cp "$repo_root/packaging/ffmpeg.lock" "$stage_root/provenance/ffmpeg.lock"
cp "$source_root/ffmpeg/COPYING.LGPLv2.1" "$stage_root/LICENSES/FFmpeg-LGPL-2.1-or-later.txt"; cp "$source_root/libvpx/LICENSE" "$stage_root/LICENSES/libvpx-BSD-3-Clause.txt"; cp "$source_root/opus/COPYING" "$stage_root/LICENSES/Opus-BSD-3-Clause.txt"; cp "$repo_root/packaging/licenses/nv-codec-headers-MIT.txt" "$stage_root/LICENSES/nv-codec-headers-MIT.txt"; cp "$repo_root/packaging/NOTICE.txt" "$stage_root/NOTICE.txt"

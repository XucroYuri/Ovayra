...
cd "$source_root/ffmpeg"; configure=(--prefix="$stage_root" --disable-autodetect --disable-debug --disable-doc --disable-ffplay --disable-network --enable-ffmpeg --enable-ffprobe --enable-libopus --enable-libvpx --enable-version3 --disable-gpl --disable-nonfree --enable-vaapi --enable-libdrm --enable-nvenc --enable-nvdec --extra-cflags="-I$dependency_prefix/include" --extra-ldflags="-L$dependency_prefix/lib"); ./configure "${configure[@]}"; make -j"$parallelism"; fate_targets=$(make fate-list | grep -E '^fate-(lavf-matroska|vp9|opus)' | head -n 3 || true); [[ -n "$fate_targets" ]] || exit 66; set -- $fate_targets; make "$@"; make install

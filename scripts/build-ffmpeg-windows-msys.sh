...
# rewrite /Brepro or /experimental:deterministic as filesystem paths before
# the compiler toolchain sees them.
configure=(--prefix="$(cygpath -m "$stage_root")" --toolchain=msvc --target-os=win32 --arch=x86_64 --ar="lib.exe -Brepro" --disable-autodetect --disable-debug --disable-doc --disable-ffplay --disable-network --enable-ffmpeg --enable-ffprobe --enable-libopus --enable-libvpx --enable-version3 --disable-gpl --disable-nonfree --enable-d3d11va --enable-dxva2 --enable-mediafoundation --enable-ffnvcodec --enable-nvenc --enable-nvdec --extra-cflags="-MD -Brepro -experimental:deterministic -I$prefix_win/include" --extra-cxxflags="-MD -Brepro -experimental:deterministic -I$prefix_win/include" --extra-ldflags="-Brepro -LIBPATH:$prefix_win/lib" --extra-libs="opus.lib vpx.lib")
